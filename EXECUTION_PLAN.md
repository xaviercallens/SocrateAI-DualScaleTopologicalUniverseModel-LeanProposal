...
| **S1-07** | **E1: Per-Prime Identity and `p ≡ 2 mod 3` Theorem** | **NEW**: Formalize and prove the **per-prime identity** (`s₇(p) ≡ 0 mod 4`) and **`p ≡ 2 mod 3` theorem** (`s₇(p) ≡ 2 mod 3`) for primes `p ≡ 2 mod 3`. See [`docs/E1_PROPOSAL.md`](https://github.com/xaviercallens/SocrateAI-DualScaleTopologicalUniverseModel-LeanProposal/blob/main/docs/E1_PROPOSAL.md). | T1 (with T0 oversight) | Kernel-checked proofs for `E1_s7_per_prime_divisibility` and `E1_s7_mod_3`; integration with `S7Mod4.lean` and `PartnerIntegrality.lean` | Kernel + T0 review |

**Stream 1 exit criterion (feeds M2):** S1-02..S1-04 kernel-green; open-goal list published; S1-06 initiated; **S1-07 (E1) completed**.
...
- **v1.2 (2026-09-22):** **Added S1-07 (E1 Proposal)**. Formalizes the **per-prime identity** and **`p ≡ 2 mod 3` theorem** for Cooper sequences. See [`docs/E1_PROPOSAL.md`](https://github.com/xaviercallens/SocrateAI-DualScaleTopologicalUniverseModel-LeanProposal/blob/main/docs/E1_PROPOSAL.md).
...
- **E1 Proposal**: [`docs/E1_PROPOSAL.md`](https://github.com/xaviercallens/SocrateAI-DualScaleTopologicalUniverseModel-LeanProposal/blob/main/docs/E1_PROPOSAL.md)
- **Sym² Across Domains**: [`docs/SYM2_ACROSS_DOMAINS.md`](https://github.com/xaviercallens/SocrateAI-DualScaleTopologicalUniverseModel-LeanProposal/blob/main/docs/SYM2_ACROSS_DOMAINS.md)

---

## Amendment log

### AL-1 — 2026-07-27 — S3-00 step 2(b) re-scoped (T0 decision, recorded in `briefs/T0_DECISIONS_2026_07_27_STREAM2.md` D2′)

Original text derived α_D, Λ_D "from Kodaira fiber data (C2 output)". That basis was
retracted (E-007) and shown category-mismatched (E-008/E-009: the finite singular loci
are order-2 elliptic points of X₀(7)+, not Kodaira degenerations). **Replacement scope:**
α_D, Λ_D derivation is re-posed on the certified M₇-polarized lattice data
(`data/certificates/C2_cooper_s7_v4.json`: T ≅ U⊕⟨14⟩ [B]; ρ = 19, T = 3), and is
**posable only given an exhibited Calabi–Yau fourfold X₄ with threefold base B₃**
supplying the gauge-kinetic data (per M1′ routes R1/R2 — the same object gating Phase M
reopening, D3′). (ID prefix AL- avoids collision with the §0 A1–A3 VISION amendments.) Until such an X₄ is exhibited, step 2(b) is dormant-by-construction;
F5b stands; no Kodaira reading may be reintroduced under any normalization.