#!/usr/bin/env python3
"""
publish_to_zenodo.py — Zenodo deposit for Stream 1 (Dual-Scale, Lean 4).

ADAPTED, not written from scratch: the HTTP layer, the `newversion` flow and the
error handling are taken verbatim from
`SocrateAI-Scientific-DualScaleSimulator/scripts/publish_to_zenodo.py`
(same author, same machine), which has a passing new-version test beside it.
Only two things changed: the files to upload come from the COMMAND LINE rather
than a hardcoded bundle list, and the metadata is read from this repo's
`.zenodo.json`.

Depositing a new version keeps the concept-DOI lineage, so
10.5281/zenodo.22853238 continues to resolve to the latest version.

⚠️ A PUBLISHED ZENODO DOI IS PERMANENT AND PUBLIC. It cannot be deleted or
un-published. Use --dry-run first, then --no-publish to stage a draft for human
review, and only then publish.

Usage:
  python3 scripts/publish_to_zenodo.py --dry-run FILE...
  python3 scripts/publish_to_zenodo.py --new-version-of RECORD_ID --no-publish FILE...
  python3 scripts/publish_to_zenodo.py --new-version-of RECORD_ID FILE...
"""
...
def main():
    ...
    parser.add_argument("files", nargs="+", help="Files to deposit.")
