...
\subsection{The embedding witness, kernel-checked}\label{subsec:embedding}

Proposition~\ref{prop:g0complement} was obtained by an exact computation outside
Lean, status (E). Its \emph{witness} --- the pair of embeddings whose existence
is what the proposition asserts --- is now status (K)
(\artifact{Agora/Geometry/Embedding.lean}).

\begin{proposition}[status (K)]\label{prop:embedding}
Let $\U^3$ carry the basis $(e_1,f_1,e_2,f_2,e_3,f_3)$ and set
\[
  B = \bigl(e_1,\; f_1,\; e_3+7f_3\bigr), \qquad
  C = \bigl(e_2,\; f_2,\; e_3-7f_3\bigr)
\]
as $6\times3$ integer matrices. Then
$B^{\mathsf T}\U^3B = \U\oplus\lat{14}$, \
$C^{\mathsf T}\U^3C = \U\oplus\lat{-14}$, \
$B^{\mathsf T}\U^3C = 0$, and $\U^3$ is an involution as a matrix, hence
unimodular. The index of the sum in $\U^3$ is $14=\lvert\operatorname{disc}
(\U\oplus\lat{14})\rvert$, so both sublattices are primitive and each is the
other's orthogonal complement.
\end{proposition}

Six dimensions suffice because $E_8(-1)$ is unimodular: the $E_8(-1)^2$ summand
of $\Lambda$ contributes nothing to the gluing, sits entirely inside the
complement, and splits off orthogonally. All the arithmetic content of
$(\U\oplus\lat{14})\perp(\U\oplus E_8(-1)^2\oplus\lat{-14})\subset\U^3\oplus E_8(-1)^2$
therefore lives in $\U^3$, and that is what is checked. The index statement is
verified where it lives, inside the third hyperbolic plane, as the determinant
$-14$ of the $2\times2$ change of basis $(e,f)\mapsto(e+7f,e-7f)$; we do not
expand a $6\times6$ determinant, which carries no content and which the library
has no lemma for. A control confirms the coefficient matters: replacing $7$ by
$6$ gives $w^2=12\neq14$ and $B$ no longer embeds $\U\oplus\lat{14}$.

\subsubsection*{The assembly, no longer on paper}

The paragraph above says that all the arithmetic content lives in $\U^3$ ``and
that is what is checked''. That left one step outside the kernel --- the
passage from the $6\times6$ statement to the rank-$22$ one, which was the
standard orthogonal-direct-sum argument, performed on paper. It is now
mechanical as well (\artifact{Agora/Geometry/EmbeddingAssembly.lean}), and it
is checked in the generality it deserves rather than only at $N=7$.

\begin{proposition}[status (K)]\label{prop:assembly}
\emph{(i) Generic orthogonal join.} Let $G$ be a symmetric matrix over a
commutative ring, and let $B,C$ be maps into it, of arbitrary index types, with
$B^{\mathsf T}GB = G_1$, $C^{\mathsf T}GC = G_2$ and $B^{\mathsf T}GC = 0$.
Then the joined map $[\,B \mid C\,]$ satisfies
\[
  [\,B \mid C\,]^{\mathsf T}\,G\,[\,B \mid C\,]
  \;=\; G_1 \oplus G_2 .
\]
The remaining cross-term $C^{\mathsf T}GB = 0$ is \emph{derived} from the
symmetry of $G$, not assumed.
\emph{(ii) Instantiation.} With $\Lambda = \U^3\oplus E_8(-1)^2$ of rank $22$,
$\Phi_T$ the embedding of $\U\oplus\lat{14}$ through $B$, and $\Phi_M$ that of
$(\U\oplus\lat{-14})\oplus E_8(-1)^2$ through $C$ and the identity,
\[
  \Phi^{\mathsf T}\Lambda\,\Phi
   \;=\; (\U\oplus\lat{14}) \;\oplus\; \bigl((\U\oplus\lat{-14})\oplus E_8(-1)^2\bigr),
  \qquad \Phi = [\,\Phi_T \mid \Phi_M\,],
\]
the vanishing off-diagonal blocks being the orthogonality assertion.
\emph{(iii)} That right-hand side has determinant $-196 = -14^2$.
\end{proposition}

Three boundaries are worth stating, because the proposition is easy to read as
saying more than it does. First, (ii) is a statement about the \emph{pullback}
of $\Lambda$ along $\Phi$; it is not a rank, injectivity or index claim about
$\Phi$ itself, and none is proved. (Since $-196\neq0$, (iii) does force $\Phi$
to have trivial kernel over $\Q$, which is what rules out degenerate solutions
of (ii); we do not formalize that consequence.) Second, the unimodularity of
$E_8(-1)$ is quoted from the lattice library and is $\mathtt{rfl}$-level over an
encoded Cartan matrix; it is inherited, not re-derived. Third --- and this
qualifies the last clause of Proposition~\ref{prop:embedding} as well --- the
passage from ``index $=$ discriminant'' to ``both sublattices are primitive and
each is the other's orthogonal complement'' is the standard Nikulin-style
criterion. What the kernel supplies is the numerical coincidence the criterion
consumes, in (iii) and in Proposition~\ref{prop:embedding}; the criterion
itself is literature and is not formalized. The summand ordering above differs
from $\U\oplus E_8(-1)^2\oplus\lat{-14}$ by a permutation of basis vectors,
which is a convention and not a mathematical difference.

What remains outside the kernel is unchanged: that the \emph{monodromy} lattice
of the $s_7$ family is $\U\oplus\lat{14}$ depends on Numerical
Claim~\ref{nc:monodromy} and stays Tier~B. What is now mechanical is that the
lattice-theoretic configuration the claim asserts does exist and is the one
displayed.
...
Two further identities pin down what $\rho$ \emph{is}, and both are
kernel-checked: $\operatorname{tr}\rho(g) = (\operatorname{tr}g)^2-\det g$, the
character of the symmetric square of the standard $2$-dimensional
representation; and $\det\rho(g) = (\det g)^3$, so an element of determinant $1$
maps to an isometry of determinant $1$. Since $\U\oplus\lat{2N}$ has signature
$(2,1)$, this exhibits $\rho$ as the integral form of the exceptional
isomorphism $\mathrm{SL}(2)\to\mathrm{SO}(2,1)$: $\Gamma_0(N)^{+}$ acts through
\emph{orientation-preserving} isometries, Atkin--Lehner elements included
($\det\rho_W(g)=(Nad-bc)^3$). This is also the precise sense in which the rank-3
lattice picture is the \emph{same} symmetric-square construction as the operator
identity of Theorem~\ref{thm:sym2-template}: the monodromy of $\Ltwo$ is a
rank-2 representation, that of $\Symtwo(\Ltwo)$ is its symmetric square, and
$\rho$ is that symmetric square written in integer coordinates. The
correspondence is classical --- it is the binary-quadratic-form picture
underlying \cite{Dolgachev1996} --- and we record it because it explains why a
$3\times3$ \emph{integer} representation exists at all.
...
\begin{remark}[Two rank-3 lattices, not one]\label{rem:two-lattices}
The classical picture and the one above act on \emph{different} lattices, and
it is worth stating the difference because merging them is an easy and
consequential error. In Gauss's coordinates a $2\times2$ matrix acts on the
coefficient triple $(a,b,c)$ of a binary quadratic form by substitution,
preserving the discriminant; for $\Gamma_0(N)$-forms $Na x^2+bxy+cy^2$ the
invariant is $b^2-4Nac$, whose Gram matrix has determinant $-4N^2$. The lattice
of Proposition~\ref{prop:rho} is $\U\oplus\lat{2N}$, of determinant $-2N$. Both
have signature $(2,1)$ and both carry integer symmetric-square actions of
$\Gamma_0(N)^{+}$, but they are \emph{not} isometric: a change of basis
multiplies the Gram determinant by a square, and $-4N^2=-2N$ only for $N=0$ or
$N=\tfrac12$, so the determinants differ for every integer $N\geq1$. The two
play different roles --- the discriminant lattice classifies forms, while
$\U\oplus\lat{2N}$ is the transcendental lattice of an $M_n$-polarized K3
\cite{Dolgachev1996} --- and they should not be named interchangeably.
Formalized as \artifact{no\_isometry\_G0N\_TN} in
\artifact{Agora/Geometry/SymSquareForms.lean}, which also carries the Gauss
presentation: $\mathrm{sym}_2(M)^{\mathsf T}G_0\,\mathrm{sym}_2(M)=(\det M)^2G_0$
with no hypothesis on $M$ (the discriminant is a relative invariant of weight
2), together with the observation that $\mathrm{sym}_2$ is
\emph{contravariant}, $\mathrm{sym}_2(MM')=\mathrm{sym}_2(M')\,\mathrm{sym}_2(M)$,
since substitution reverses composition.
\end{remark}

\begin{remark}[The signature, now mechanical]\label{rem:signature-tierA}
The signature $(2,1)$ of $\U\oplus\lat{2N}$, quoted above and earlier recorded
as an asserted value, is now checked: the integer basis change
$P=(e+f,\;w,\;e-f)$, of determinant $2$, gives
$P^{\mathsf T}(\U\oplus\lat{2N})P=\operatorname{diag}(2,2N,-2)$, so for $N>0$
there are two positive and one negative direction, Sylvester's law of inertia
(quoted) supplying the rest. Formalized as \artifact{TN\_diagonalises}, with a
control confirming the final sign is $-2$ and not $+2$.
\end{remark}
