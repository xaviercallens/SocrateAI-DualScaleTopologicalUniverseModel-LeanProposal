...
$W\equiv 0$ holds identically on the template. We also prove, again
kernel-checked and uniformly in the parameters, the \emph{sequence-level} form
of the factorization: the recurrence-defined partner sequence is the formal
square root of the order-3 sequence, so its coefficients lie in
$\Z[\tfrac12]$. The partner separates the three
...
\input{sections/03-operators}
\input{sections/04-sym2}
\input{sections/05-irreducibility}
\input{sections/06-lattice}
\input{sections/07-integrality}
\input{sections/08-dolgachev-doran}
\input{sections/09-formalization}
\input{sections/10-reproducibility}
\input{sections/10a-calabi-yau-construction}
\input{sections/10b-empirical-astrophysics}
...
\end{document}
% ---------------------------------------------------------------------------
% Generated-by: Fable 5 (Stream 1 paper agent); \S8 G0/G1 additions and
% \S11 Acknowledgments added 2026-07-29 by Sonnet 5 (WP-P1); \S9.4
% (three-strikes discipline), \S8 U+<20> stub, and \S10a/\S10b empty
% placeholder sections added 2026-07-30 by Sonnet 5 (T0 Directive 3) |
% Verified-by: pdflatex+bibtex build, claims traced per paper/PLAN.md #3 (row
% 21 added for G0); \S10a/\S10b carry zero mathematical or physical content
% (nothing to verify beyond their own absence of claims) | Reviewed-by:
% pending T0 (Xavier)
% ---------------------------------------------------------------------------
